...
| 动态壁纸 Live wallpaper | 路线 B：壁纸窗口注入桌面图标层之下；组件模式（内容区可点击）+ 图片模式（`Mode=Image` 逐像素 alpha 遮罩穿透）<br>*Route B: wallpaper window injected below the desktop icon layer; Component mode (clickable content) + Image mode (`Mode=Image` pixel-alpha mask click-through)* |
