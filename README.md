...
| 层级区分 Layering            | 同位置多组件经 `ZIndex=`（写在 `[Prismica]` 节或 `layout.ini`）在运行时排序置顶；`ZOrderArranger` 升序稳定排序 + `SetZOrder(HWND_TOP)`，保证叠加顺序可预期<br>*Same-position multi-widget layering via `ZIndex=` (in `[Prismica]` or `layout.ini`); `ZOrderArranger` ascending stable sort + `SetZOrder(HWND_TOP)` for deterministic stacking* |
